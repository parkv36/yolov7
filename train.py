...
def get_model(cfg, nc, anchors, device='0'):
    return Model(cfg, ch=3, nc=nc, anchors=anchors).to(device)
...
def train(hyp, opt, device, tb_writer=None):
    ...
    assert mlc < nc_complete, 'Label class %g exceeds nc=%g in %s. Possible class labels are 0-%g' % (mlc, nc_complete, opt.data, nc - 1)
...
def run_train(opt):
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ckpt = opt.resume if isinstance(opt.resume, str) else get_latest_run(opt.project)  # specified or most recent path
        assert os.path.isfile(ckpt), 'ERROR: --resume checkpoint does not exist'
        apriori = opt.global_rank, opt.local_rank, opt.epochs, opt.generators, opt.preprocessing_function, opt.model
        with open(Path(ckpt).parent.parent / 'opt.yaml') as f:
            opt = argparse.Namespace(**yaml.load(f, Loader=yaml.SafeLoader))  # replace
        opt.cfg, opt.weights, opt.resume, opt.batch_size, opt.global_rank, opt.local_rank, opt.epochs, opt.generators, opt.preprocessing_function, opt.model = '', ckpt, True, opt.total_batch_size, *apriori  # reinstate
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {'lr0': (1, 1e-5, 1e-1),  # initial learning rate (SGD=1E-2, Adam=1E-3)
                ...
                'fl_alpha': (0, 0.05, 0.95),  # focal loss alpha (efficientDet default gamma=0.25)
